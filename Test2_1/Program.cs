...
public class Answer
 {
  public static double[, ] CreateRandomMatrix(int m, int n, int minLimitRandom, int maxLimitRandom, int max)
  {
      double[,] matrix= new double [minLimitRandom, maxLimitRandom];
        for(int i = 0; i < array.GetLength(0); i++)
          for(int k = 0; k < array.GetLength(1); k++)
            matrix[i, k] =  new Random.Next(minLimitRandom, maxLimitRandom); 
  ...
  public static void PrintArray(double[, ] matrix)
  {
    ...
    {
        for (int p = 0; p < matrix[p].length; p++) 
        {
            Console.WriteLine(matrix[l,p] + " ");
        }
        Console.WriteLine();
    }
